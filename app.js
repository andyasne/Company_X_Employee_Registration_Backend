 ...
 * @module              app.js
 * @description         Starting point for companyXEmployeeRegistration service.
 * @kind                Init
 * @copyright           2019
 */
...
let
    ...
    debug       = require('debug')('companyXEmployeeRegistration'),
...
app.use('/employee/apidoc'    ,express.static(path.join(__dirname,'./apidocs')));
app.use('/employee/coverage'  ,express.static(path.join(__dirname,'./coverage/lcov-report')));
app.use('/employee/test'      ,express.static(path.join(__dirname,'./report')));
...
app.use(function (req, res, next) {
    if(req.url === "/") {
        res.redirect('http://localhost:3300/employee/apidoc');
        res.status(200);
    }else{
